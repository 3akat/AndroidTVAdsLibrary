...
import static com.example.androidtvlibrary.main.adapter.AdPlaybackStateFactoryTest.MICROS_PER_SECOND;
import static com.example.androidtvlibrary.main.adapter.AdPlaybackStateTest.checkArgument;
...
//@UnstableApi
public final class ImaAdsLoaderTest implements TestPlayer.EventListener, AdsLoaderTest {
...
//        ExoPlayerLibraryInfo.registerModule("goog.exo.ima");
    ...
    public static final class Builder {
        ...
        public static final long DEFAULT_AD_PRELOAD_TIMEOUT_MS = 10 * MILLIS_PER_SECOND;
        ...
        public Builder setAdUiElements(Set<UiElement> adUiElements) {
            this.adUiElements = new HashSet<>(adUiElements);
            return this;
        }
...
//            Assertions.checkArgument(bitrate > 0);
        ...
        public ImaAdsLoaderTest buildForAdTag(Uri adTagUri) {
            return new ImaAdsLoaderTest(
                    context,
                    adTagUri,
                    imaSdkSettings,
                    /* adsResponse= */ null,
        ...
        public ImaAdsLoaderTest buildForAdsResponse(String adsResponse) {
            return new ImaAdsLoaderTest(
                    context,
                    /* adTagUri= */ null,
                    imaSdkSettings,
    ...
    private static final String IMA_SDK_SETTINGS_PLAYER_VERSION = "";
    ...
    @Nullable
    private TestException pendingAdLoadError;
    private Timeline timeline;
    private long contentDurationMs;
    private AdPlaybackStateTest adPlaybackState;
    ...
    public ImaAdsLoaderTest(Context context, Uri adTagUri) {
        this(
                context,
                adTagUri,
                /* imaSdkSettings= */ null,
...
//        Assertions.checkArgument(adTagUri != null || adsResponse != null);
        ...
        handler = createHandler(getImaLooper(), /* callback= */ null);
        ...
        adPlaybackState = AdPlaybackStateTest.NONE;
    }
...
//        Assertions.checkState(player == null || player.getApplicationLooper() == getImaLooper());
    ...
    public void start(EventListener eventListener, AdViewProvider adViewProvider) {
        ...
        if (hasAdPlaybackState) {
            ...
            adPlaybackState = AdPlaybackStateFactoryTest.fromCuePoints(adsManager.getAdCuePoints());
    ...
    public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
        ...
        if (playbackState == TestPlayer.STATE_BUFFERING && !player.isPlayingAd()) {
            // Check whether we are waiting for an ad to preload.
            int adGroupIndex = getLoadingAdGroupIndex();
            if (adGroupIndex == C.INDEX_UNSET) {
                return;
            }
            AdPlaybackStateTest.AdGroup adGroup = adPlaybackState.adGroups[adGroupIndex];
            if (adGroup.count != C.LENGTH_UNSET
                    && adGroup.count != 0
                    && adGroup.states[0] != AdPlaybackStateTest.AD_STATE_UNAVAILABLE) {
                // An ad is available already so we must be buffering for some other reason.
                return;
            }
            long adGroupTimeMs = usToMs(adPlaybackState.adGroupTimesUs[adGroupIndex]);
        ...
        } else if (playbackState == TestPlayer.STATE_READY) {
            waitingForPreloadElapsedRealtimeMs = C.TIME_UNSET;
        }
...
//    }
    ...
    private AdsRenderingSettings setupAdsRendering() {
        ...
        if (adGroupForPositionIndex != C.INDEX_UNSET) {
            boolean playAdWhenStartingPlayback =
                    playAdBeforeStartPosition
                            || adGroupTimesUs[adGroupForPositionIndex] == msToUs(contentPositionMs);
            ...
            if (adGroupForPositionIndex > 0) {
                ...
                if (adGroupForPositionTimeUs == C.TIME_END_OF_SOURCE) {
                    ...
                    adsRenderingSettings.setPlayAdsAfterTime(
                            (double) adGroupBeforePositionTimeUs / MICROS_PER_SECOND + 1d);
                } else {
                    ...
                    adsRenderingSettings.setPlayAdsAfterTime(midpointTimeUs / MICROS_PER_SECOND);
                }
            }
        }
            ...
            case AD_BREAK_FETCH_ERROR:
                ...
                int adGroupIndex =
                        adGroupTimeSeconds == -1
                                ...
                                : linearSearch(
                                adPlaybackState.adGroupTimesUs, MICROS_PER_SECOND * adGroupTimeSeconds);
                AdPlaybackStateTest.AdGroup adGroup = adPlaybackState.adGroups[adGroupIndex];
                if (adGroup.count == C.LENGTH_UNSET) {
                    adPlaybackState =
                ...
                for (int i = 0; i < adGroup.count; i++) {
                    if (adGroup.states[i] == AdPlaybackStateTest.AD_STATE_UNAVAILABLE) {
                        if (DEBUG) {
    ...
    private void updateAdProgress() {
        ...
        AdMediaInfo adMediaInfo = imaAdMediaInfo;
        for (int i = 0; i < adCallbacks.size(); i++) {
    ...
    private void handleTimelineOrPositionChanged() {
        ...
        if (!sentContentComplete && !wasPlayingAd && playingAd && imaAdState == IMA_AD_STATE_NONE) {
            ...
            if (adPlaybackState.adGroupTimesUs[adGroupIndex] == C.TIME_END_OF_SOURCE) {
                ...
                fakeContentProgressOffsetMs = usToMs(adPlaybackState.adGroupTimesUs[adGroupIndex]);
...
//        Assertions.checkNotNull(imaAdInfo);
    ...
    private void handleAdGroupLoadError(Exception error) {
        ...
        if (pendingAdLoadError == null) {
            pendingAdLoadError = TestException.createForAdGroup(error, adGroupIndex);
        }
        pendingContentPositionMs = C.TIME_UNSET;
        fakeContentProgressElapsedRealtimeMs = C.TIME_UNSET;
    }
    ...
    private void maybeNotifyInternalError(String name, Exception cause) {
        ...
        if (eventListener != null) {
            eventListener.onAdLoadError(
                    TestException.createForUnexpected(new RuntimeException(message, cause)),
    ...
    private int getAdGroupIndexForAdPod(AdPodInfo adPodInfo) {
        ...
        long adPodTimeUs =
                Math.round((double) ((float) adPodInfo.getTimeOffset()) * MICROS_PER_SECOND);
    ...
    private int getLoadingAdGroupIndex() {
        long playerPositionUs =
                msToUs(getContentPeriodPositionMs(player, timeline, period));
        int adGroupIndex =
    ...
    private static DataSpecTest getAdsDataSpec(@Nullable Uri adTagUri) {
        return new DataSpecTest(adTagUri != null ? adTagUri : Uri.EMPTY);
    }

    private static long getContentPeriodPositionMs(
            TestPlayer player, Timeline timeline, Timeline.Period period) {
    ...
    private final class ComponentListener
        ...
        public boolean areEqual(@Nullable Object o1, @Nullable Object o2) {
        ...
        public void onAdsManagerLoaded(AdsManagerLoadedEvent adsManagerLoadedEvent) {
            ...
            if (!areEqual(pendingAdRequestContext, adsManagerLoadedEvent.getUserRequestContext())) {
                adsManager.destroy();
                return;
            }
            pendingAdRequestContext = null;
            ImaAdsLoaderTest.this.adsManager = adsManager;
...
//                pendingAdLoadError = AdLoadException.createForAllAds(error);
        ...
        public void loadAd(AdMediaInfo adMediaInfo, AdPodInfo adPodInfo) {
            try {
                ...
                AdPlaybackStateTest.AdGroup adGroup = adPlaybackState.adGroups[adInfo.adGroupIndex];
                adPlaybackState =
